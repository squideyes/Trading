...
public class TickSet : ListBase<Tick>
{
    ...
    public static readonly MajorMinor Version = new(3, 0);

    private TickOn? lastTickOn = null;

    public TickSet(Source source, Pair pair, DateOnly tradeDate)
    {
        ...
        Session = new Session(Extent.Day, tradeDate);
    }
    ...
    public string GetFileName(DataKind dataKind)
    {
        ...
        sb.AppendDelimited(Session.TradeDate.ToString("yyyyMMdd"), '_');
        sb.Append('_');
        sb.Append("EST");
        sb.Append('.');
    ...
    public void SaveToStream(Stream stream, DataKind dataKind)
    {
        ...
        else
        {
            ...
            writer.Write(Session.Extent.ToString());
            writer.Write(Session.TradeDate.DayNumber);
            writer.Write(Count);
    ...
    public void LoadFromStream(Stream stream, DataKind dataKind)
    {
        ...
        if (dataKind == DataKind.CSV)
        {
            ...
            {
                var tickOn = new TickOn(DateTime.Parse(fields[0]));
        ...
        else
        {
            ...
            if (reader.ReadString().ToEnumValue<Extent>() != Session.Extent)
                throw new ArgumentOutOfRangeException(nameof(stream));

            var tradeDate = DateOnly.FromDayNumber(reader.ReadInt32());

            if (tradeDate != Session.TradeDate)
                throw new ArgumentOutOfRangeException(nameof(stream));
            ...
            for (var i = 1; i < count; i++)
            {
                ...
                {
                    tickOn = lastTick.TickOn.Value.AddMilliseconds(
                        header & MINIFIEDTICKONMASK);
    ...
    public static TickSet Create(string fileName)
    {
        ...
        var tradeDate = DateOnly.ParseExact(fields[2], "yyyyMMdd", null);
        ...
        return new TickSet(source, pair, tradeDate);
    }
}